...
while True:
    ...
    #    cv2.imshow("undistorted", undistorted.asarray(np.float32) / 4500.)
    ...
    #if enable_rgb and enable_depth:
        #registered1 = registered.asarray(np.uint8)
        # discard the 4th channel
        #registered1 = registered1[:, :, :3]  # not even gonna use color1 anymore, but this is good to know
        # grab the frame dimensions and convert it to a blob
        #(h, w) = color.asarray(np.uint8)[:, :, :3].shape[:2]
        # print(h,w)
        #blob = cv2.dnn.blobFromImage(cv2.resize(registered.asarray(np.uint8)[:, :, :3], (300, 300)),0.007843, (300, 300), 127.5)
        ...
        #for i in np.arange(0, detections.shape[2]):
            ...
            # greater than the minimum confidence
                ...
                # print(label)
                #cv2.rectangle(registered.asarray(np.uint8), (startX, startY), (endX, endY),COLORS[idx], 2)
                # y = startY - 15 if startY - 15 > 15 else startY + 15
                # cv2.putText(color.asarray(np.uint8), label, (startX, y),cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS[idx], 2)

        # show the output frame
        #cv2.imshow("Frame", registered.asarray(np.uint8))
